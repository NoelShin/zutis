...
def extract_text_embeddings(
    ...
    for category in tqdm(categories):
        ...
        if len(templates) == 1:
            cat_to_text_embedding[category] = text_embeddings.squeeze(dim=0).to(torch.float32)
        else:
...
def prompt_engineering(
    ...
    cat_to_text_embedding: Dict[str, torch.FloatTensor] = extract_text_embeddings(
        model=model, categories=categories, templates=prompt_templates if use_templates else ["{}"]
    )
